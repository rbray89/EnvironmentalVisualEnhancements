...
Ported to c# for generic use in unity.
Original Copyright:
...
namespace Equirectangular2Cubic
{
    public class CubicGen
    {

        static private double[][] mt;
        static private long[][] mi;
        const int MI_MULT = 4096;
        const int MI_SHIFT = 12;
        const int NATAN = 65536;
        const int NSQRT = 65536;
        const int NSQRT_SHIFT = 16;
        ...
        static uint UNIT_XSAMPLES = 256;
        // number of subdivisions of the y-axis unity
        static uint UNIT_YSAMPLES = 1024;
        ...
        public static Cubemap getCubic(Texture2D inputTex)
        {
            ...
            uint[][] equiData = new uint[height][];
            for (int i = 0; i < height; i++)
            {
                equiData[i] = new uint[width];
            }
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    uint color = (uint)((pixels[pixel].a<<24)+(pixels[pixel].r<<16)+(pixels[pixel].g<<8)+ pixels[pixel].b);
                    equiData[y][x] = color;
                    pixel++;
                }
            }
            Log("initialized color");
            uint[][] cubic = processImageFile(equiData, ref width, ref height);
            Log("initialized newcubic");
            Cubemap newCubic = new Cubemap(width, TextureFormat.ARGB32, true);
            Log("initialized newcubic");
            newCubic.SetPixels(GetPixels(cubic[0]), CubemapFace.PositiveZ);
            newCubic.SetPixels(GetPixels(cubic[1]), CubemapFace.PositiveX);
            newCubic.SetPixels(GetPixels(cubic[2]), CubemapFace.NegativeZ);
        ...
        {
            UnityEngine.Debug.Log("E2C: " + message);
        }

        private static Color[] GetPixels(uint[] p)
        {
            Color[] newColors = new Color[p.Length];

            for (int i = 0; i < p.Length; i++ )
            {
                float r = ((p[i] >> 16) & 0x00FF) / 255.0f;
                float g = ((p[i] >> 8) & 0x00FF) / 255.0f;
                float b = (p[i] & 0x00FF) / 255.0f;
                float a = ((p[i] >> 24) & 0x00FF) / 255.0f;
                newColors[i] = new Color(r,g,b,a);
            }
            return newColors;
        }

        static private int getNextPowerOf2(int equiHeight)
        {
            uint test = 0x80000000;
            while ((equiHeight & test) == 0)
            {
                test >>= 1;
            }
            test >>= 1;
        ...
        public static uint[][] processImageFile(uint[][] equiData, ref int rectWidth, ref int rectHeight)
        {

            int equiWidth = rectWidth;
            int overlap = 0;
            double fov; // horizontal field of view
            double r = rectWidth / (2D * Math.PI);
            double y = (Math.Tan(Math.PI / 4D) * r + overlap);
            fov = Math.Atan(y / r) * 180 / Math.PI * 2;
            //rectWidth = (int)(y * 2);
            rectWidth = getNextPowerOf2(rectWidth);
            rectHeight = rectWidth;
            

            Log("setting interpolation");
            Boolean bilinear;
            Boolean lanczos2;
            String interpolation = "lanczos2";
            ...
            yaw = 0;
            pitch = 0;
            rectData[0] = extractRectilinear(yaw, pitch, fov, equiData, rectWidth, rectHeight, equiWidth, bilinear, lanczos2);


            yaw = 90;
            pitch = 0;
            rectData[1] = extractRectilinear(yaw, pitch, fov, equiData, rectWidth, rectHeight, equiWidth, bilinear, lanczos2);


            yaw = 180;
            pitch = 0;
            rectData[2] = extractRectilinear(yaw, pitch, fov, equiData, rectWidth, rectHeight, equiWidth, bilinear, lanczos2);


            yaw = 270;
            pitch = 0;
            rectData[3] = extractRectilinear(yaw, pitch, fov, equiData, rectWidth, rectHeight, equiWidth, bilinear, lanczos2);


            yaw = 0;
            pitch = 90;
            rectData[4] = extractRectilinear(yaw, pitch, fov, equiData, rectWidth, rectHeight, equiWidth, bilinear, lanczos2);


            yaw = 0;
            pitch = -90;
            rectData[5] = extractRectilinear(yaw, pitch, fov, equiData, rectWidth, rectHeight, equiWidth, bilinear, lanczos2);
        ...
        static public void initForIntArray2D(uint[][] intArray2D)
        {
            math_setLookUp(intArray2D);
        }

        static public uint[] extractRectilinear(
            double yaw,
            double pitch,
            double fov,
            uint[][] equiData,
            ...
            Boolean lanczos2)
        {
            uint[] returnRectData = new uint[rectWidth * rectHeight];
            math_extractview(
                equiData, //ai1,
        ...
        private static void math_setLookUp(uint[][] ai)
        {
            if (ai != null)
            {
                if (atan_LU_HR == null)
        ...
        // 	
        private static uint bilinear_interp_pixel(uint p00, uint p01, uint p10, uint p11, int dx, int dy)
        {
            int k1 = 255 - dx;
            int l1 = 255 - dy;
            int i2 = k1 * l1;
            ...
            long k3 =
                i2 * (p00 & 0xff)
                    ...
                    + j2 * (p10 & 0xff)
                    + k2 * (p11 & 0xff) >> 16;
            return (uint)(i3 + (j3 << 8) + k3 + 0xff000000);
        }
        // 	
        private static void SetMatrix(double d, double d1, double[][] ad, int i)
        {
            ...
            {
                ad1[index] = new double[3];
                ad2[index] = new double[3];
            }
            ad1[0][0] = 1.0D;
            ad1[0][1] = 0.0D;
            ad1[0][2] = 0.0D;
        ...
        private static void lanczos2_init()
        {
            ...
            lanczos2_weights_LU = new int[UNIT_XSAMPLES + 1][];
            for (int i = 0; i < UNIT_XSAMPLES + 1; i++)
            {
                lanczos2_weights_LU[i] = new int[MAX_WEIGHTS];
            }
            // allocates temporary buffers
            aR = new int[MAX_WEIGHTS];
            aG = new int[MAX_WEIGHTS];
            aB = new int[MAX_WEIGHTS];
            aA = new int[MAX_WEIGHTS];
        }
        // 
        ...
        private static void lanczos2_compute_weights(double pscale)
        {
            ...
            for (int j = 0; j <= UNIT_XSAMPLES; j++)
            {
                ...
                int k;
                s = 0;
                long i = j + UNIT_XSAMPLES * (lanczos2_n_points - 1);
        ...
        static uint lanczos2_interp_pixel(
            uint[][] pd,
            int pw,
            int ph,
            int xs,
            ...
            int dy)
        {
            ...
            return (uint)((tmpA << 24) + (tmpR << 16) + (tmpG << 8) + tmpB);
        }
